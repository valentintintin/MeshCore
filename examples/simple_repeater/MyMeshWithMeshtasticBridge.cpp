...
void MyMeshWithMeshtasticBridge::text_message_callback(uint32_t from, uint32_t to,  uint8_t channel, const char* text) {
  instance->text_message_received(from, to, channel, text);
}

void MyMeshWithMeshtasticBridge::want_to_send_to_meshtastic(const char *send_name, const char *text, int text_len) {
...
MyMeshWithMeshtasticBridge::MyMeshWithMeshtasticBridge(mesh::MainBoard& board, mesh::Radio& radio, mesh::MillisecondClock& ms, mesh::RNG& rng, mesh::RTCClock& rtc, mesh::MeshTables& tables)
...
#define PUBLIC_GROUP_PSK                "izOH6cXN6mrJ5e26oRXNcg=="
  ...
  if (len == 32 || len == 16) {
    mesh::Utils::sha256(meshcore_public_channel.hash, sizeof(meshcore_public_channel.hash), meshcore_public_channel.secret, len);
  }
  ...
  mesh::Utils::sha256(meshcore_test_channel.hash, sizeof(meshcore_test_channel.hash), meshcore_test_channel.secret, len);
}

bool MyMeshWithMeshtasticBridge::begin(FILESYSTEM *fs, const int8_t rxPin, const int8_t txPin, const uint32_t baudRate) {
  MyMesh::begin(fs);
...
bool MyMeshWithMeshtasticBridge::add_channel(uint8_t index, const char *name) {
  MESH_DEBUG_PRINTLN("Bridge MT. Add channel %s at position %d", name, index);

  if (strcasecmp_P(name, PSTR("Public")) == 0) {
    memset(meshcore_public_channel.secret, 0, sizeof(meshcore_public_channel.secret));
    const auto psk_base64 = PUBLIC_GROUP_PSK;
    const int len = decode_base64((unsigned char *) psk_base64, strlen(psk_base64), meshcore_public_channel.secret);
    if (len == 32 || len == 16) {
      mesh::Utils::sha256(meshcore_public_channel.hash, sizeof(meshcore_public_channel.hash), meshcore_public_channel.secret, len);
...
void MyMeshWithMeshtasticBridge::add_node_info(mt_node_t *nodeinfo, mt_nr_progress_t progress) {
  MESH_DEBUG_PRINTLN("Bridge MT receive node info progress: %d. Node id: !%x, %s -> %s", progress, nodeinfo->node_num, nodeinfo->short_name, nodeinfo->long_name);
...
void MyMeshWithMeshtasticBridge::text_message_received(uint32_t from, uint32_t to, uint8_t channel, const char *text) {
  MESH_DEBUG_PRINTLN("Bridge MT receive a text message on channel: %d from !%x to !%x: %s", channel, from, to, text);
...
#define MAX_TEXT_LEN    (10*CIPHER_BLOCK_SIZE)  // must be LESS than (MAX_PACKET_PAYLOAD - 4 - CIPHER_MAC_SIZE - 1)
  ...
  const auto pkt = createGroupDatagram(PAYLOAD_TYPE_GRP_TXT, meshcore_test_channel, temp, 5 + prefix_len + text_len);
...
int MyMeshWithMeshtasticBridge::searchChannelsByHash(const uint8_t* hash, mesh::GroupChannel dest[], int max_matches) {
  int n = 0;
  ...
  if (!n) {
    MESH_DEBUG_PRINTLN("Message from unknown channel: %x. Public: %x. Test: %x", hash[0], meshcore_public_channel.hash[0], meshcore_test_channel.hash[0]);
  }

  return n;
}

void MyMeshWithMeshtasticBridge::onGroupDataRecv(mesh::Packet* packet, uint8_t type, const mesh::GroupChannel& channel, uint8_t* data, size_t len) {
  MESH_DEBUG_PRINTLN("Bridge MT received from MC a packet: %d from channel: %x of size: %d", type, channel.hash[0], len);
...
void MyMeshWithMeshtasticBridge::onChannelMessageRecv(const mesh::GroupChannel &channel, mesh::Packet *pkt, uint32_t timestamp, const char *text) {
