...
bool MyMeshWithMeshtasticBridge::begin(FILESYSTEM *fs) {
  MyMesh::begin(fs);

  add_channel(0, "public");
  add_channel(1, "#valentintest");

  if (_meshtastic_bridge_prefs.rx_pin != -1 && _meshtastic_bridge_prefs.tx_pin != -1 && _meshtastic_bridge_prefs.baud_rate > 0) {
    MESH_DEBUG_PRINTLN("Bridge MT init on pin TX: %d and RX: %d with baudRate: %d", _meshtastic_bridge_prefs.tx_pin, _meshtastic_bridge_prefs.rx_pin, _meshtastic_bridge_prefs.baud_rate);
...
#define MT_SERIAL Serial1
    ...
    MT_SERIAL.begin(_meshtastic_bridge_prefs.baud_rate, SERIAL_8N1, _meshtastic_bridge_prefs.rx_pin, _meshtastic_bridge_prefs.tx_pin);
#elif defined(ARDUINO_ARCH_RP2040)
    MT_SERIAL.setPinout(_meshtastic_bridge_prefs.tx_pin, _meshtastic_bridge_prefs.rx_pin);
#ifdef SERIAL_HW_FIFO_SIZE
    MT_SERIAL.setFIFOSize(SERIAL_HW_FIFO_SIZE);
#endif
    MT_SERIAL.begin(_meshtastic_bridge_prefs.baud_rate);
#else
    MT_SERIAL.begin(_meshtastic_bridge_prefs.baud_rate);
#endif
...
void MyMeshWithMeshtasticBridge::add_node_info(mt_node_t *nodeinfo, mt_nr_progress_t progress) {
  ...
  if (progress == MT_NR_IN_PROGRESS) {
    node_infos[node_infos_count++].node_num = nodeinfo->node_num;
    strcpy(node_infos[node_infos_count++].long_name, nodeinfo->long_name);
  }
}
...
void MyMeshWithMeshtasticBridge::text_message_received(uint32_t from, uint32_t to, uint8_t channel,
  ...
  if (to == 0xFFFFFFFF) {
    ...
    if (node_info != nullptr) {
      snprintf(sender_name, sizeof(sender_name), "MT_%s", node_info->long_name);
    } else {
      snprintf(sender_name, sizeof(sender_name), "MT!%x", from);
    }

    send_to_meshcore(sender_name, text, strlen(text));
  }
}
...
#define MAX_TEXT_LEN                                                                           \
  ...
  mesh::GroupChannel *channel = &_meshtastic_bridge_prefs.channels[0].channel;
  for (auto channel : _meshtastic_bridge_prefs.channels) {
    if (channel)
  }

  const auto pkt = createGroupDatagram(PAYLOAD_TYPE_GRP_TXT, channel, temp, 5 + prefix_len + text_len);
...
int MyMeshWithMeshtasticBridge::searchChannelsByHash(const uint8_t *hash, mesh::GroupChannel dest[],
  ...
  for (auto channel : _meshtastic_bridge_prefs.channels) {
    if (channel.channel.hash[0] == hash[0] && n < max_matches) {
      MESH_DEBUG_PRINTLN("Message from %s channel", channel.name);
      dest[n++] = channel.channel;
    }
  }

  if (!n) {
    MESH_DEBUG_PRINTLN("Message from unknown channel: %x", hash[0]);
  }
...
void MyMeshWithMeshtasticBridge::onChannelMessageRecv(const mesh::GroupChannel &channel, mesh::Packet *pkt,
  ...
  want_to_send_to_meshtastic("MC", text, channel);
}